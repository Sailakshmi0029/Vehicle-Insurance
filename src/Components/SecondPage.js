...
const SecondPage = () => {
  ...
  const handleSendMobileOTP = async () => {
    if (/^[6-9]\d{9}$/.test(formData.mobile)) {
      try {
        const response = await axios.get(`http://192.168.1.200:9090/customer/get/${formData.mobile}`);
  ...
  const handleSendEmailOTP = async () => {
    if (/^[a-z0-9._%+-]+@gmail\.com$/.test(formData.email)) {
      try {
        const response = await axios.get(`http://192.168.1.200:9090/customer/getByEmail/${formData.email}`);
        ...
        if (formData.email===response.data.email) {
          ...
          try {
            const response = await axios.post(`http://192.168.1.200:9090/vehicle/sendEmailOTPforUpdation/${formData.email}`);
      ...
      setEmailOtpError('Please enter a valid mobile number');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://192.168.1.200:9090/customer/add`, formData, {
        headers: {
                  ...
                  <div className="sp-form-group ">
                    ...
                    {isMobileOTPSent && (
                      <div>
                        ...
                        <button type="button" className="sp-otp-verify-button" onClick={() => handleVerifyOTP('mobile')}>Verify OTP</button>
                      </div>
                    )}
                  </div>
                  <div className="sp-form-group">
                    ...
                    {isEmailOTPSent && (
                      <div>
                        ...
                        <button type="button" className="sp-otp-verify-button" onClick={() => handleVerifyOTP('email')}>Verify OTP</button>
                      </div>
                    )}
                  </div>
